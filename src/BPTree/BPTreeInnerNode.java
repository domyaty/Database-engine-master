...
public class BPTreeInnerNode<T extends Comparable<T>> extends BPTreeNode<T>  implements Serializable
{
	...
	public BPTreeLeafNode<T> getFirstLeaf() {
		return children[0].getFirstLeaf();
	}
	
	public BPTreeLeafNode<T> getLeafNode(T key) {
		return children[findIndex(key)].getLeafNode(key);
	}
	...
	{
		return children[findIndex(key)].contain(key);
	}
